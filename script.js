...
const databases={
    "Relational":[
        {name:"MYSQL",tags:["relation","consistent_immediate"]},
        {name:"Postgres",tags:["relation","consistent_immediate"]},
        {name:"Oracle",tags:["relation","consistent_immediate"]},
        {name:"Aurora",tags:["relation","consistent_immediate"]}
    ],
    "Graph":[
        {name:"Neptune",tags:["graph"]}
    ],
    "Key Value":[
        {name:"DynamoDB",tags:["keyvalue","consistent_immediate"]}
    ],
    "Memory":[
        {name:"Memcached",tags:["memory"]},
        {name:"Redis",tags:["memory"]}
    ],
    "Document":[
        {name:"MongoDB",tags:["document"]},
        {name:"DocumentDB",tags:["document"]}
    ],
    "Time Series":[
        {name:"Timeseries",tags:["timeseries"]}
    ],
    "Ledger":[
        {name:"QLDB",tags:["ledger"]}
    ]
};
...
function ToggleByQuestion(category,question){
    ...
    //Add new tag
    question.tags.forEach(tag => filtersTags.push({"category":category,tag:tag}))
    generateDatabaseOutput();
}
...
Object.keys(questions).forEach(category => {
    ...
    questions[category].forEach((question)=>{
        ...
        questionElement.innerHTML="<input id='questionid"+questionID+"' type='radio' name='"+category+"'/>"
        
    
       questionElement.innerHTML+="<label for='questionid"+questionID+"'>"+question.name+"</label>";
        questionID+=1;
